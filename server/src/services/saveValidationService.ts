...
export function extractCoins(profile: Record<string, unknown> | null | undefined): number {
  if (!profile) {
    return 0;
  }

  const homeWorld = profile.homeWorld as { Coin?: number } | undefined;
  const coins = homeWorld?.Coin;
  return typeof coins === "number" && Number.isFinite(coins) ? Math.max(0, Math.floor(coins)) : 0;
}

function elapsedMinutes(from: Date | null | undefined, now: Date): number {
  if (!from) {
    return 1;
  }
...
export function validateCoinDelta(
