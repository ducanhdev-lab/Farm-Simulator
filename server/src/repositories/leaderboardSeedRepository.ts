...
const DEMO_ENTRIES: { playerId: string; coins: number; label: string }[] = [
...
export function isDemoLeaderboardSeedingEnabled(): boolean {
...
export async function seedDemoLeaderboard(): Promise<number> {
    ...
    await pool.query(
      `INSERT INTO players (id, save_version, profile_json, updated_at)
       ...
       ON CONFLICT (id) DO UPDATE SET
         ...
         updated_at = NOW()`,
      [entry.playerId, JSON.stringify(profile)]
    );
    ...
    await pool.query(
      `INSERT INTO island_snapshots (player_id, snapshot_json, updated_at)
       ...
       ON CONFLICT (player_id) DO UPDATE SET
         ...
         updated_at = NOW()`,
      [entry.playerId, JSON.stringify(snapshot)]
    );
